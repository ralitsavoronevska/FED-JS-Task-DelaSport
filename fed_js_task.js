...
// Task 1 Solution
// Function for getting all the keys from marketCategories.rules Object and store them in an Array.
const getKeysFromMarketCategories = (marketCategoriesObj) => {
  // The reduce() method executes a reducer function (that you provide) on each element of the Array, resulting in single output value.
  const KeysFromMarketCategories = marketCategoriesObj.rules.reduce((a,b) => {
    a.push(b.key);
    return a;
  },[]);
    return KeysFromMarketCategories;
}

const filterMarkets = (marketsObj) => {
  ...
  const objectToArray = Object.entries(marketsObj);

  // The filter() method creates a new Array with all elements that pass the test implemented by the provided function.
  const filterArray = objectToArray.filter(f => {
    //reusing getKeysFromMarketCategories(marketCategories) in order to get all the keys in an Array.
    const allKeysFromMarketCatecories = getKeysFromMarketCategories(marketCategories);
      // If the value with index 0 (f[0]) in markets Array exists as key in allKeysFromMarketCatecories the used method returns true and adds the values in the new Array.
      // The includes() method determines whether an array includes a certain value among its entries, returning true or false as appropriate.
    const resultFromFilter2 = allKeysFromMarketCatecories.includes(f[0]);

    return resultFromFilter2; 
      console.log(allKeysFromMarketCatecories); 
  });
  
  // Transforming the Array to an Object
  const arrayToObject = filterArray.reduce((accum, [k, v]) => {
      accum[k] = v;
      return accum;
    }, {})
...
// Task 2 Solution
const sortMarket = (marketsObj) => {

  // The Object.entries() method returns an array of a given object's own enumerable string-keyed property [key, value] pairs, in the same order as that provided by a for...in loop.
  const objectToArray = Object.entries(marketsObj);

  // The sort() method sorts the elements of an array in place and returns the sorted array. The default sort order is ascending, built upon converting the elements into strings, then comparing their sequences of UTF-16 code units values.
  const sortArray = objectToArray.sort((a,b)=>a[1].info.order - b[1].info.order);

  // Transforming the Array to an Object
  const arrayToObject = sortArray.reduce((accum, [k, v]) => {
      accum[k] = v;
      return accum;
    }, {});
...
const matchedMarkets = filterMarkets(markets);
const sortedMarkets = sortMarket(matchedMarkets);
console.log(matchedMarkets);
